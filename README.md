...
台灣裁判語義檢索服務（22,578,975 筆，截至 2026-09-13）,讓你能用自然語言搜尋判決,並將檢索結果帶入自己的 AI 工具使用。

## 資料涵蓋（統計截至 2026-09-13，取自 production 資料庫）

**一、法規範（依法位階）**

| 位階 | 資料類型 | 數量 | 說明 |
|---|---|---:|---|
| 憲法 | 憲法位階 | 7 部／228 條 | 憲法本文、增修條文、實施準備程序及解嚴令等現行規範 |
| 憲法 | 釋字與憲判字 | 870 筆 | 大法官解釋 813 筆、憲法法庭判決 57 筆，有拘束全國各機關及人民之效力 |
| 法律 | 法律 | 1,017 部／44,372 條 | 定名為法、律、條例或通則者（中央法規標準法 §2） |
| 命令 | 法規命令 | 7,249 部／128,675 條 | 定名為規程、規則、細則、辦法、綱要、標準或準則者（同法 §3） |
| 命令 | 行政規則 | 88,421 筆 | 90 個機關發布之解釋性規定與裁量基準（行政程序法 §159），對外不生法規範效力 |
| 沿革 | 已廢止法規 | 3,523 部 | 法律 320 部、法規命令 3,201 部，及動員戡亂時期臨時條款等憲法位階規範，標示廢止狀態供查考 |

**二、裁判與裁決**

| 資料類型 | 數量 | 說明 |
|---|---:|---|
| 裁判書（司法院各級法院） | 22,578,975 筆 | 語義檢索＋詞彙檢索＋案號精確調卷；每日增量同步司法院公開資料，含事後更正與下架之同步對齊 |
...
| 資料類型 | 數量 | 說明 |
|---|---:|---|
| 審級關聯（上訴鏈） | 4,540,466 筆 | 隨判決附 `case_history`，含主文「廢棄／駁回」旗標 |
| 函釋效力履歷 | 69,509 筆 | 廢止／停止適用／被取代狀態追蹤，引用前驗效力 |

法規範與裁判之取用方式：裁判書、行政規則走 hosted MCP 的語義檢索與字號精確查詢；
憲法、法律、法規命令於 [dr-legal.com.tw](https://dr-legal.com.tw) 站上檢索。
...
### 行政規則發布機關明細（90 個機關，統計日 2026-09-13）
...
- **22,578,975 筆**台灣裁判資料（截至 2026-09-13），經過結構化處理與向量化。
...
- **現行法條精確查詢**:以法規名稱＋條號取回現行條文原文,並附法規最後修正
  日期與廢止註記。引用法條前可先核驗條號存在與文字內容,不必憑模型記憶背條文;
  常用縮寫（勞基法、刑法、憲法）自動解析為官方法規名。僅提供現行整編版本,
  行為時法（修法前舊條文）請至官方沿革查詢。
...
- **Gemini 網頁版（Spark）**：開啟 [gemini.google.com/spark/apps](https://gemini.google.com/spark/apps)
  ...
  需帳號具備 Gemini Spark;連結後在 Spark 輸入框打 `@` 選「法律偵探」即可指名呼叫,一般聊天視窗不會呼叫。
  2026-09-01 台灣帳號實測可用。
